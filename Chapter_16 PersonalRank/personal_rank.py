...
def PersonalRank(data_dict, alpha, user, maxCycles):
	...
	while step < maxCycles:
		...
		for i, ri in list(data_dict.items()):
			for j in list(ri.keys()):
				if j not in tmp:
					tmp[j] = 0
...
def recommend(data_dict, rank, user):
	...
	# 1、用户user已打过分的项
	items = []
	for k in list(data_dict[user].keys()):
		items.append(k)
	...
	# 3、按打分的降序排序
	result = sorted(list(items_dict.items()), key=lambda d: d[1], reverse=True)
	return result
